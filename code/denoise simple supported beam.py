...
class PhysicsInformedNN:
    ...
    def __init__(self, xd, td, wd, N, layers, lb, ub):
        ...
        self.ei = tf.Variable(1.0, name='stiffness')
        ...
        # Loss
        self.loss = 1000 * (tf.reduce_mean(tf.square(self.wd_tf - self.wd_pred)) + \
        ...
        self.train_op_Adam = self.optimizer_Adam.minimize(self.loss+self.loss2)
...
if __name__ == "__main__": 
    ...
    """
    try:
        np.save('w_pred', W_pred)
        np.save('w_d', W_d)
        np.save('w_exact', W_exact)
        np.save('fw_pred', FW_pred)
        np.save('d_w', D_W)
    except:
        pass
    """
    ...
    w = ax.imshow(W_pred.T, interpolation='nearest', cmap='YlGnBu', 
                  ...
                  origin='lower', aspect='auto', vmin=-np.amax(np.abs(W_pred)), vmax=np.amax(np.abs(W_pred)))
    ...
    ax.set_xlabel('$t$')
    ax.set_ylabel('$x$')
    ax.set_title('Exact $w(x,t)$', fontsize = 10)
    ...
    ax.set_xlabel('$t$')
    ax.set_ylabel('$x$')
    ax.set_title('Noised $w(x,t)$, SNR = %s dB, %s sensors' % (snr,res), fontsize = 10)
    ...
    w = ax.imshow(D_W.T, interpolation='nearest', cmap='seismic',
                  extent=[lb[1], ub[1], lb[0], ub[0]], 
                  origin='lower', aspect='auto', norm=DivergingNorm(0.), vmin=-np.amax(np.abs(D_W)), vmax=np.amax(np.abs(D_W)))
    ...
    w = ax.imshow(FW_pred.T, interpolation='nearest', cmap='PuOr', 
                  extent=[lb[1], ub[1], lb[0], ub[0]], 
                  origin='lower', aspect='auto', norm=DivergingNorm(0.), vmin=-np.amax(np.abs(FW_pred)), vmax=np.amax(np.abs(FW_pred)))
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(w, cax=cax)
