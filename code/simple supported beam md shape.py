...
class PhysicsInformedNN:
    ...
    def __init__(self, exact_w, exact_w_t, N, layers, lb, ub):
        ...
        self.loss = tf.reduce_max((self.w0_tf - self.w0_pred)**2)
    ...
    def initialize_NN(self, layers):
        #W = tf.Variable(tf.truncated_normal([layers[1],]), dtype=tf.float32)
        W = tf.Variable(0.5*tf.ones([layers[1],]), dtype=tf.float32)
        n = tf.range(1, layers[1]+1, 1, dtype=tf.float32)
        return W, n
    ...
    def neural_net(self, x, t, w, n):
        Y = tf.reduce_sum(w * tf.sin(n*np.pi*x) * tf.cos(2*(n*np.pi)**2*t), axis=1)
        return Y
    
    def net_w(self, x, t):

        w = self.neural_net(x, t, self.weights, self.biases)
    ...
    def grid_points(self, n_x=150, n_t=150):
        ...
        #x0 = np.array([[0.25], [0.5], [0.75]]); t0 = np.zeros(x0.shape)
    ...
    def train(self, nIter):
        ...
        for it in range(nIter):
            tf_dict = self.grid_points(self.N[0], self.N[1])
        ...
        """
        tf_dict = self.grid_points(10000, 150)
...
if __name__ == "__main__": 
    ...
    N_0 = 500 # number of initial points
