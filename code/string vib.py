...
class PhysicsInformedNN:
    ...
    def __init__(self, exact_w, exact_w_t, N, layers, lb, ub):
        ...
        # tf Graphs
        self.w0_pred, self.w_t0_pred, _, _ = self.net_w(self.x0_tf, self.t0_tf) # initial points prediction
        self.w_lb_pred, self.w_tlb_pred, _, _ = self.net_w(self.x_lb_tf, self.t_lb_tf) # lb points prediction
        self.w_ub_pred, self.w_tub_pred, _, _ = self.net_w(self.x_ub_tf, self.t_ub_tf) # ub points prediction
    ...
    def net_w(self, x, t):
        ...
        return w, w_t, w_x, w_xx

    def net_f_w(self, x, t):
        _, w_t, _, w_xx = self.net_w(x,t)
...
if __name__ == "__main__": 
    ...
    Exact_w = lambda x, t: np.sin(np.pi*x) * np.cos(np.pi*np.sqrt(1-(0.5/np.pi)**2)*t) * np.exp(-1/2 * t)
    Exact_w_t = lambda x, t: - 0.5*np.exp(-1/2*t)*np.sin(np.pi*x) * np.cos(np.pi*np.sqrt(1-(0.5/np.pi)**2)*t) + \
        - np.pi*np.sqrt(1-(0.5/np.pi)**2) * np.sin(np.pi*x) * np.sin(np.pi*np.sqrt(1-(0.5/np.pi)**2)*t) * np.exp(-1/2 * t)
    ...
    ax4.plot(x,W_exact[50,:], 'b-', linewidth = 2, label = 'Exact')       
    ax4.plot(x,W_pred[50,:], 'r--', linewidth = 2, label = 'Prediction')
    ax4.set_xlabel('$x$'); ax4.set_ylabel('$w(x,t)$')    
    ax4.set_title('$t = %.2f$' % (t[50]), fontsize = 10)
    ax4.set_aspect(0.5)
    ...
    try:
        savefig('string ' + str(layers[1:-1]))
    except:
        pass 

    try:
        savefig('./figures/string ' + str(layers[1:-1]))
    except:
        pass 
