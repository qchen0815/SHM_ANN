...
if __name__ == "__main__": 
    ...
    mu = 4.694
    Exact_w = lambda x, t: (cosh(mu*x) - cos(mu*x) - (cosh(mu) + cos(mu)) / (sinh(mu) + sin(mu)) * (sinh(mu*x) - sin(mu*x))) * cos(mu**2*t)
    Exact_w_t = lambda x, t: 0 * x
    ...
    try:
        savefig('CT ' + str(layers[1:-1]))
    except:
        pass 

    try:
        savefig('./figures/CT ' + str(layers[1:-1]))
    except:
        pass 
