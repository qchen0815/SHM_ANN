...
class PhysicsInformedNN:
    ...
    def __init__(self, xd, td, wd, tb, X_f, layers, lb, ub):
        ...
        self.loss = tf.reduce_mean(tf.square(self.wd_tf - self.wd_pred)) + \
                    ...
                    1/3 * tf.reduce_mean(tf.square(self.w_x_lb_pred)) + \
                    1/10 * tf.reduce_mean(tf.square(self.w_xx_ub_pred)) + \
                    1/33 * tf.reduce_mean(tf.square(self.w_xxx_ub_pred)) + \
                    1/100 * tf.reduce_mean(tf.square(self.f_w_pred))
...
#    plt.setp(leg.get_texts(), color='w')
    ...
    ax.legend(loc='upper center', bbox_to_anchor=(0.4, -0.8), ncol=5, frameon=False)
