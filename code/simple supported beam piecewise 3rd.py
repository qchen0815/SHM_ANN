...
class PhysicsInformedNN:
    ...
    def __init__(self, w_0, w_t0, layers, lb, ub):
        ...
        self.loss_g = tf.reduce_mean(tf.square(self.f_w_pred)) / 5000 + \
        ...
        self.optimizer2 = tf.contrib.opt.ScipyOptimizerInterface(self.loss+self.loss_g, 
                                                                method = 'L-BFGS-B', 
    ...
    def neural_net(self, X, weights, biases):
        ...
        for l in range(0,num_layers-2):
            ...
            b = biases[l]
            if l > 0:
                H = H + tf.sin(tf.add(tf.matmul(H, W), b))
            else:
...
if __name__ == "__main__":      
    ...
    for n in range(steps):
        ...
        t = np.linspace(lb[1], ub[1]/steps, 50)[:,None]
        X, T = np.meshgrid(x,t)
