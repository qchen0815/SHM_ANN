...
class PhysicsInformedNN:
    ...
    def __init__(self, xd, td, wd, N, layers, lb, ub):
        ...
        # tf Graphs
        self.wd_pred, _, _ = self.net_w(self.xd_tf, self.td_tf) # initial points prediction
        ...
        self.loss = 1e4 * tf.reduce_mean(tf.square(self.wd_tf - self.wd_pred)) + \
                    1e4 * tf.reduce_mean(tf.square(self.w_lb_pred)) + \
                    tf.reduce_mean(tf.square(self.v1_lb_pred)) + \
                    1e4 * tf.reduce_mean(tf.square(self.w_ub_pred)) + \
...
if __name__ == "__main__": 
    ...
    try:
        savefig('d SSB A' + str(layers[1:-1]))
    except:
        pass 

    try:
        savefig('./figures/d SSB A' + str(layers[1:-1]))
    except:
        pass 
