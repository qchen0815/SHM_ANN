...
class PhysicsInformedNN:
    ...
    def __init__(self, exact_w, exact_w_t, N, layers, lb, ub):
        ...
        self.w_lb_pred, _, _, _ = self.net_w(self.x_lb_tf, self.t_lb_tf) # lb points prediction
        self.w_ub_pred, _, _, _ = self.net_w(self.x_ub_tf, self.t_ub_tf) # ub points prediction
...
if __name__ == "__main__": 
    ...
    Exact_w = lambda x, t: np.sin(np.pi*(x-t)) * np.heaviside(np.pi*(x-t), 0)
    Exact_w_t = lambda x, t: - 2*np.pi * np.sin(2*np.pi*x) * np.sin(2*np.pi*t)
    ...
    try:
        savefig('pde ' + str(layers[1:-1]))
    except:
        pass 

    try:
        savefig('./figures/pde ' + str(layers[1:-1]))
    except:
        pass 
