...
New network combining 2 networks to avoid trivial solution.
"""
...
class PhysicsInformedNN:
    ...
    def __init__(self, xd, td, wd, tb, X_f, layers, lb, ub):
        ...
        self.weights_c, self.biases_c = self.initialize_NN([2, 100, 100, 100, 1], 0.02)
        ...
        self.w_lb_pred, self.w_x_lb_pred = self.net_w_lb(self.x_lb_tf, self.t_lb_tf) # lb points prediction
        self.w_xx_ub_pred, self.w_xxx_ub_pred = self.net_w_ub(self.x_ub_tf, self.t_ub_tf) # ub points prediction
        ...
        self.loss = tf.reduce_mean(tf.square(self.wd_tf - self.wd_pred)) + \
                    ...
                    #tf.reduce_mean(tf.square(self.w_xxx_ub_pred)) + \
                    #tf.reduce_mean(tf.square(self.f_w_pred))
        self.loss_2 = self.loss + tf.reduce_mean(tf.square(self.f_w_pred))
        ...
        self.optimizer_phase2 = tf.contrib.opt.ScipyOptimizerInterface(self.loss_2, 
                                                                method = 'L-BFGS-B',
                                                                var_list=[*self.weights_c, *self.biases_c],
                                                                options = {'maxiter': 50000,
                                                                           'maxfun': 50000,
                                                                           'maxcor': 50,
                                                                           'maxls': 50,
        ...
        self.train_op_Adam_phase2 = self.optimizer_Adam.minimize(self.loss_2, var_list=[*self.weights_c, *self.biases_c])
    ...
    def initialize_NN(self, layers, sig=2):        
        weights = []
        biases = []
        num_layers = len(layers) 
        for l in range(0,num_layers-1):
            W = self.xavier_init(size=[layers[l], layers[l+1]], sig=sig)
            b = tf.Variable(tf.zeros([1,layers[l+1]], dtype=tf.float32), dtype=tf.float32)
            weights.append(W)
            biases.append(b)        
        return weights, biases
        
    def xavier_init(self, size, sig=2):
        in_dim = size[0]
        out_dim = size[1]        
        xavier_stddev = np.sqrt(sig/(in_dim + out_dim))
    ...
    def net_w(self, x, t):
        ...
        c = self.neural_net(X, self.weights_c, self.biases_c)

        return w + c
    
    def net_w_lb(self, x, t):
        w = self.net_w(x, t)
        w_x = tf.gradients(w, x)[0]
        
        return w, w_x
    
    def net_w_ub(self, x, t):
        w = self.net_w(x, t)
        w_x = tf.gradients(w, x)[0]
    ...
    def train(self, nIter):
        ...
        start_time = time.time()
        for it in range(nIter):
            self.sess.run(self.train_op_Adam_phase2, tf_dict)
            
            # Print
            if it % 10 == 0:
                elapsed = time.time() - start_time
                loss_value = self.sess.run(self.loss_2, tf_dict)
                print('It: %d, Loss: %.3e, Time: %.2f' % 
                      (it, loss_value, elapsed))
                start_time = time.time()
        
        self.optimizer_phase2.minimize(self.sess, 
                                feed_dict = tf_dict,         
                                fetches = [self.loss_2], 
                                loss_callback = self.callback)
