...
class PhysicsInformedNN:
    ...
    def __init__(self, xd, td, wd, N, layers, lb, ub):
        ...
        self.weights2, self.biases2 = self.initialize_NN([1, 500, 1])
        ...
        self.EI = self.net_EI(self.x_f_tf, self.weights2, self.biases2)
        self.f_w_pred = self.net_f_w(self.x_f_tf, self.t_f_tf, self.EI) # governing equation
        
        # Loss

        self.loss = 5000 * (tf.reduce_mean(tf.square(self.wd_tf - self.wd_pred)) + \
                            5 * tf.reduce_mean(tf.square(self.w_lb_pred)) + \
                            5 * tf.reduce_mean(tf.square(self.w_ub_pred)))
        self.loss2 = tf.reduce_mean(tf.square(self.w_xx_lb_pred)) + \
                     tf.reduce_mean(tf.square(self.w_xx_ub_pred)) + \
                     tf.reduce_mean(tf.square(self.f_w_pred))

        # Optimizers
        self.optimizer = tf.contrib.opt.ScipyOptimizerInterface(self.loss, #+self.loss2, 
                                                                method = 'L-BFGS-B', 
    ...
    def net_EI(self, x, weights, biases):
        num_layers = len(weights) + 1
        ...
        W = weights[-1]
        b = biases[-1]
        Y = 1 - tf.sigmoid(tf.add(tf.matmul(H, W), b))
        return Y
    ...
    def predict(self, X_star):
        ...
        ei_star = self.sess.run(self.EI, tf_dict)
               
        return w_star, ei_star

def exact_data(case=8, freq=0.01, space=30):
    data = np.loadtxt('../data/damaged/case{:0>2d}/displacement.txt'.format(case), skiprows=9)
    data = data[data[:,0].argsort()][1:-1]
    #index = np.arange(0, 2500, freq/(0.01/25), dtype=int)
    index = np.arange(0, 500)
    displacement = data[:, index+2].flatten()[:, None]
    loc = data[:, 0].flatten()
    time = np.linspace(0, 1, 2501)[index]
    time, loc = np.meshgrid(time, loc)
    time = time.flatten()[:, None]
    loc = loc.flatten()[:, None]
    return loc, time, displacement
...
if __name__ == "__main__": 
    ...
    x_exact, t_exact, w_exact = exact_data(8)
