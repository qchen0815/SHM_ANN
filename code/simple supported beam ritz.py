...
class PhysicsInformedNN:
    ...
    def __init__(self, exact_w, exact_w_t, N, layers, lb, ub):
        ...
        self.x_g, self.t_g, self.w_g = self.gauss_coef(500)
        ...
        # Loss
        self.loss = 1e10 * (tf.reduce_mean(tf.square(self.w0_tf - self.w0_pred)) + \
                    tf.reduce_mean(tf.square(self.w_t0_tf - self.w_t0_pred))) + \
                    tf.reduce_sum(self.w_g * self.f_w_pred)
                    #tf.reduce_mean(tf.square(self.w_lb_pred)) + \
                    #tf.reduce_mean(tf.square(self.w_ub_pred))) + \
                    #tf.reduce_sum([tf.reduce_sum(tensor**2) for tensor in self.weights])
    ...
    def rand_points(self, n_0, n_b):
        ...
        tf_dict = {
            ...
            self.x_f_tf: self.x_g, self.t_f_tf: self.t_g
            }
    ...
    def gauss_coef(self, deg=100):
        xg, wg = leggauss(deg)
        x = ((self.ub[0]-self.lb[0])/2*xg + (self.ub[0]+self.lb[0])/2)
        t = ((self.ub[1]-self.lb[1])/2*xg + (self.ub[1]+self.lb[1])/2)
        x, t = np.meshgrid(x, t)
        x = x.flatten()[:, None]; t = t.flatten()[:, None]
        wx = (self.ub[0]-self.lb[0])/2*wg
        wt = (self.ub[1]-self.lb[1])/2*wg[:, None]
        w = (wx*wt).flatten()[:, None]
        return x, t, w
    ...
    def train(self, nIter):
        ...
        for it in range(nIter):
            ...
            if it % 10 == 0:
                ...
                start_time = time.time()
        """
        tf_dict = self.grid_points(250, 250)                                                                                   
...
if __name__ == "__main__": 
    ...
    try:
        savefig('R SSB ' + str(layers[1:-1]))
    except:
        pass 

    try:
        savefig('./figures/R SSB ' + str(layers[1:-1]))
    except:
        pass 
