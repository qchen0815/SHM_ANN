"""
@author: Qiuyi Chen
Code for dulplicating the paper of Alli.
The tensorflow gradient algorithm failed to derive the gradient of phi1 wrt x. This might be the cause of failure.
Some manual derivation of the high order gradient might be brought in to overcome this issue.
"""
...
class PhysicsInformedNN:
    ...
    def net_phi1(self, t):
        ...
        phi_1 = self.neural_net(X, self.weights, self.biases)
        phi_x1 = tf.gradients(phi_1, x1)[0]

        return phi_1, phi_x1

    def net_w(self, x, t):
        phi = self.net_phi(x, t)
        phi1, phi_x1 = self.net_phi1(t)
        w = (1 - t**2) * tf.sin(pi*x) + \
            x**3 * (x - 1) * t**2 * (phi - phi1 - 1/3*phi_x1)
        return phi_x1
