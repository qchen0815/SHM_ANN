...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, w0, w_t0, tb, X_f, layers, lb, ub):
        ...
        self.w_lb_pred, _, self.w_x_lb_pred, _, _ = self.net_w(self.x_lb_tf, self.t_lb_tf) # lb points prediction
        _, _, _, self.w_xx_ub_pred, self.w_xxx_ub_pred = self.net_w(self.x_ub_tf, self.t_ub_tf) # ub points prediction
        ...
        self.loss = 1000* (tf.reduce_mean(tf.square(self.w0_tf - self.w0_pred))) #+ \
    ...
    def xavier_init(self, size):
        ...
        return tf.Variable(tf.truncated_normal([in_dim, out_dim], stddev=xavier_stddev)*20, dtype=tf.float32)
...
if __name__ == "__main__": 
    ...
    w_t_star = Exact_w_t.T.flatten()[:,None]
    ...
    model = PhysicsInformedNN(x0, w0, w_t0, tb, X_f, layers, lb, ub) 
